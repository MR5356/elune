<script setup>
import { ref } from 'vue'
import { listKubernetes, addKubernetes, updateKubernetes, deleteKubernetes } from '@/request/devops'
...
async function init() {
  const res = await withLoading(listKubernetes, '加载中', 'title')
  clusters.value = res.sort((a, b) => {
    return a.title.localeCompare(b.title)
  })
}
init()

async function onClickSubmitCluster() {
  if (isEdit.value) {
    await withLoading(updateKubernetes, '更新中', newCluster.value)
  } else {
    await withLoading(addKubernetes, '添加中', newCluster.value)
  }
...
function onEditCluster(cluster) {
  isEdit.value = true
  newCluster.value = JSON.parse(JSON.stringify(cluster))
  showAddCluster.value = true
}

async function onClickDelete(cluster) {
  ElMessageBox.confirm('删除所选集群', '提示', {
    confirmButtonText: '确认',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    await withLoading(deleteKubernetes, '删除中', cluster)
    await init()
  })
}
</script>
...
<template>
  ...
  >
    ...
    >
      <el-form :model="newCluster" label-position="top">
        <el-form-item label="集群名称">
          <el-input v-model="newCluster.title" />
        </el-form-item>
        <el-form-item label="集群描述">
          <el-input v-model="newCluster.desc" />
        </el-form-item>
        <el-form-item label="KubeConfig">
          <el-input v-model="newCluster.kubeConfig" type="textarea" rows="5" />
        </el-form-item>
      </el-form>
      <template #footer>
        <span>
          <el-button @click="showAddCluster = false">取消</el-button>
          <el-button type="primary" @click="onClickSubmitCluster">确认</el-button>
        </span>
      </template>
    </el-dialog>
    ...
    <div class="grid xl:grid-cols-3 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-4">
          ...
          >
            <div class="flex justify-between items-center">
              <div class="flex-col flex gap-1">
                ...
                <div class="text-xs text-gray-600">{{ cluster.desc }}</div>
              </div>
            ...
            <div class="flex justify-between">
              <div class="flex gap-2 items-center flex-wrap">
                <div
                  class="break-keep text-xs text-gray-600 bg-green-200 px-2 py-[2px] rounded-full"
                  v-if="cluster.status === 'ready'"
                >
                  已就绪
                </div>
                <div
                  class="break-keep text-xs text-gray-600 bg-red-200 px-2 py-[2px] rounded-full"
                  v-if="cluster.status === 'error'"
                >
                  集群连接失败
                </div>
                <div
                  class="break-keep text-xs text-gray-600 bg-yellow-200 px-2 py-[2px] rounded-full"
                  v-if="cluster.status === 'timeout'"
                >
                  集群连接超时
                </div>
                <div class="break-keep text-xs text-gray-600 bg-sky-200 px-2 py-[2px] rounded-full">
                  {{ cluster.version }}
                </div>
                <div class="break-keep text-xs text-gray-600 bg-sky-200 px-2 py-[2px] rounded-full">
                  {{ moment(cluster.createdAt).format('YYYY-MM-DD HH:mm:ss') }}
                </div>
              </div>
            </div>
          </div>
        ...
        <template #default>
          <div class="flex flex-col gap-0 text-sm">
            <div
              ...
              @click="onEditCluster(cluster)"
            >
              编辑
            </div>
            <div
              class="w-full hover:bg-gray-50 p-1.5 rounded cursor-pointer"
              @click="onClickDelete(cluster.id)"
            >
              删除
            </div>
          </div>
      ...
      >
        <div class="m-auto flex items-center gap-6">
          ...
          <div class="text-xl text-gray-400">添加集群</div>
        </div>
      </div>
    </div>
  </div>
